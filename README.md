...
| Áudio bidirecional | Microfone dos dois lados (44,1 kHz, mono, 16 bits), com botão de mudo |
...
| Atalhos | `Ctrl+S` iniciar/parar, `Ctrl+M` mudo, `Ctrl+Q` sair |
...
│   ├── captura_audio.py         # Captura e reprodução (PyAudio) com fila anti-latência
...
│   ├── janela_servidor.py       # Janela do host
│   └── janela_cliente.py        # Janela do espectador
├── utilitarios/
│   ├── registro.py              # Log rotativo em arquivo e console
│   ├── rede.py                  # IP local, validações e formatação de taxas
│   └── recursos.py              # Ícones e caminhos dentro do executável
├── testes/                      # 45 testes automatizados (unitários + integração)
├── build/                       # Scripts de build e especificação PyInstaller
...
- PortAudio para áudio (no Ubuntu/Debian: `sudo apt install portaudio19-dev`)
...
Se a instalação do **PyAudio** falhar, o aplicativo continua funcionando com vídeo e chat; o áudio é desativado automaticamente e avisado na interface.
...
- `teste_integracao.py` — handshake, senha correta/incorreta, chat bidirecional, vídeo, sessão exclusiva e desconexão

Os testes de integração usam dublês para captura de tela e áudio, portanto rodam em qualquer ambiente (inclusive em CI sem monitor).
...
| `Áudio indisponível` na interface | PyAudio ou PortAudio ausente | instale o PortAudio e reinstale o PyAudio |
